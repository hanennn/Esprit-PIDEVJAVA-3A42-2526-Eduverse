...
public class AjouterQuizController {
    ...
    public void initialize() {
        ...
        quizTable.setRowFactory(tv -> {
            ...
            row.selectedProperty().addListener((obs, wasSelected, isSelected) -> {
                if (!row.isEmpty())
                    row.setStyle(isSelected ? "-fx-background-color: #fff3e0;"
                            : row.getIndex() % 2 == 0 ? "-fx-background-color: white;"
                            : "-fx-background-color: #fafafa;");
            });
            return row;
        });
        ...
        chargerCours(); // ← filtrés par formateur connecté
        chargerQuiz();  // ← filtrés par formateur connecté
        configurerFiltres();
    ...
    // ─────────── CHARGER COURS (filtrés par formateur connecté) ───────────
    private void chargerCours() {
        try {
            Connection cnx = MyConnection.getInstance().getCnx();
            // ← WHERE createur_id = formateur connecté
            String sql = "SELECT id, titre_cours FROM cours WHERE createur_id = ?";
    ...
    public void chargerQuiz() {
        try {
            // afficher() retourne uniquement les quiz des cours du formateur connecté
    ...
    private void configurerFiltres() {
        ...
        searchDureeMinField.textProperty().addListener((obs, o, n) -> appliquerFiltres());
        searchDureeMaxField.textProperty().addListener((obs, o, n) -> appliquerFiltres());
        searchScoreMinField.textProperty().addListener((obs, o, n) -> appliquerFiltres());
    ...
    private void appliquerFiltres() {
        ...
        filteredList.setPredicate(quiz -> {
            ...
            if (!dureeMinStr.isEmpty()) {
                try {
                    if (quiz.getDuree() < Integer.parseInt(dureeMinStr)) return false;
            ...
            if (!dureeMaxStr.isEmpty()) {
                try {
                    if (quiz.getDuree() > Integer.parseInt(dureeMaxStr)) return false;
                } catch (NumberFormatException ignored) {}
            }
            ...
            if (!scoreMinStr.isEmpty()) {
                try {
                    if (quiz.getScoreMinimum() < Float.parseFloat(scoreMinStr)) return false;
                } catch (NumberFormatException ignored) {}
            }
    ...
    private void mettreAJourCompteur() {
        ...
        if (filtres == total) {
            ...
            resultCountLabel.setStyle("-fx-font-size: 11; -fx-text-fill: #888;");
        } else {
            resultCountLabel.setText(filtres + " résultat(s) sur " + total);
            resultCountLabel.setStyle("-fx-font-size: 11; -fx-text-fill: #f5a623; -fx-font-weight: bold;");
        }
    }
    ...
    private boolean validerChamps() {
        ...
        if (dureeField.getText().trim().isEmpty()) {
            ...
            try {
                ...
                if (d <= 0) { dureeField.setStyle(ERROR); dureeError.setText("⚠ La durée doit être > 0"); ok = false; }
            } catch (NumberFormatException e) {
                dureeField.setStyle(ERROR); dureeError.setText("⚠ La durée doit être un entier"); ok = false;
            }
        }
        if (scoreField.getText().trim().isEmpty()) {
            ...
            try {
                ...
                if (s < 0) { scoreField.setStyle(ERROR); scoreError.setText("⚠ Le score ne peut pas être négatif"); ok = false; }
            } catch (NumberFormatException e) {
                scoreField.setStyle(ERROR); scoreError.setText("⚠ Score invalide (ex: 10.5)"); ok = false;
            }
        }
    ...
    private void afficherPopupDoublon(String message) {
        ...
        root.setStyle("-fx-background-color: white; -fx-padding: 35; -fx-min-width: 380;");

        Label icone = new Label("⚠️");
        icone.setStyle("-fx-font-size: 45;");
        Label titre = new Label("Doublon détecté !");
        titre.setStyle("-fx-font-size: 18; -fx-font-weight: bold; -fx-text-fill: #e74c3c;");
        HBox barre = new HBox();
        barre.setPrefHeight(3); barre.setPrefWidth(300);
        barre.setStyle("-fx-background-color: #e74c3c; -fx-background-radius: 2;");
        Label msg = new Label(message);
        msg.setStyle("-fx-font-size: 13; -fx-text-fill: #555; -fx-text-alignment: center;");
        msg.setWrapText(true); msg.setAlignment(Pos.CENTER);
        Button btnOk = new Button("Compris !");
        btnOk.setPrefWidth(160);
        btnOk.setStyle("-fx-background-color: #e74c3c; -fx-text-fill: white;" +
    ...
    private void afficherPopupSucces(String message) {
        ...
        titre.setStyle("-fx-font-size: 18; -fx-font-weight: bold; -fx-text-fill: #2ecc71;");
        HBox barre = new HBox();
        barre.setPrefHeight(3); barre.setPrefWidth(300);
        barre.setStyle("-fx-background-color: #2ecc71; -fx-background-radius: 2;");
        Label msg = new Label(message);
        msg.setStyle("-fx-font-size: 13; -fx-text-fill: #555; -fx-text-alignment: center;");
        msg.setWrapText(true); msg.setAlignment(Pos.CENTER);
        Button btnOk = new Button("OK");
        btnOk.setPrefWidth(160);
        btnOk.setStyle("-fx-background-color: #2ecc71; -fx-text-fill: white;" +
                "-fx-font-weight: bold; -fx-font-size: 13;" +
                "-fx-background-radius: 8; -fx-padding: 10 20; -fx-cursor: hand;");
        btnOk.setOnAction(e -> popup.close());
        root.getChildren().addAll(icone, titre, barre, msg, btnOk);
    ...
    private void ajouterQuiz() {
        ...
        try {
            ...
            afficherPopupSucces("Quiz \"" + quiz.getTitre() + "\" ajouté avec succès !");
            annuler();
            chargerQuiz();
        } catch (Exception e) {
    ...
    private void ajouterEtOuvrirQuestions() {
        ...
        try {
            ...
            if (inserted == null) { showError("Impossible de récupérer le quiz !"); return; }
    ...
    private void modifierQuiz() {
        ...
        if (selected == null) { showError("Sélectionnez un quiz à modifier !"); return; }
        if (!validerChamps()) return;
        try {
            ...
            selected.setScoreMinimum(Float.parseFloat(scoreField.getText().trim()));
            selected.setCoursAssocieId(coursMap.get(coursBox.getValue()));
            service.modifier(selected);
            afficherPopupSucces("Quiz \"" + selected.getTitre() + "\" modifié avec succès !");
            annuler();
            chargerQuiz();
        } catch (Exception e) {
            if (e.getMessage() != null && e.getMessage().contains("existe déjà"))
    ...
    private void supprimerQuiz() {
        Quiz selected = quizTable.getSelectionModel().getSelectedItem();
        if (selected == null) { showError("Sélectionnez un quiz à supprimer !"); return; }
        Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
        alert.setTitle("Confirmation");
        alert.setHeaderText("Supprimer le quiz ?");
        alert.setContentText("Voulez-vous supprimer \"" + selected.getTitre() + "\" ?");
        alert.showAndWait().ifPresent(response -> {
            if (response == ButtonType.OK) {
                try {
                    service.supprimer(selected.getId());
                    afficherPopupSucces("Quiz \"" + selected.getTitre() + "\" supprimé avec succès !");
                    annuler();
                    chargerQuiz();
    ...
    private void goQuestions() {
        Quiz selected = quizTable.getSelectionModel().getSelectedItem();
        if (selected != null) naviguerQuestions(selected.getId(), selected.getTitre());
        else naviguerQuestions(0, "—");
    }
    ...
    private void naviguerQuestions(int quizId, String quizTitre) {
        ...
        } catch (Exception e) { showError("Erreur navigation : " + e.getMessage()); }
    }
