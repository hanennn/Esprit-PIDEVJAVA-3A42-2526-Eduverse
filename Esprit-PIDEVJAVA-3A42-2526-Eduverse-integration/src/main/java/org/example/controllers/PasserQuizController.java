...
public class PasserQuizController {
    ...
    @FXML private Button      btnDefinirGlobal; // ← bouton global unique
    ...
    // ══ Méthode appelée par le bouton global "Définir un mot" dans le FXML ══
    @FXML
    private void ouvrirDefinirMot() {
        double x = btnDefinirGlobal.localToScreen(btnDefinirGlobal.getBoundsInLocal()).getMinX();
        double y = btnDefinirGlobal.localToScreen(btnDefinirGlobal.getBoundsInLocal()).getMaxY();
    ...
    private VBox creerBlocQuestion(int numero, Question q) {
        ...
        // ══ BARRE DE TRADUCTION (sans le bouton "Définir un mot") ══
        ...
        tradBox.getChildren().addAll(tradIcon, langBox, btnTrad, btnReset);
