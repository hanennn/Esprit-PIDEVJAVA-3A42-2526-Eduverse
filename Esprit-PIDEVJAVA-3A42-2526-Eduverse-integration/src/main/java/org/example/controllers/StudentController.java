...
public class StudentController {
    ...
    private void chargerBadges() {
        // find the badges container — it's the VBox inside the "Badges & Réalisations" section
        // we need to add fx:id="badgesContainer" to that VBox in the FXML first
        // For now, locate it by rebuilding it programmatically:
        try {
            int userId = Session.getCurrentUser().getId();
            List<CertificationFinale> badges = studentService.getBadgesForStudent(userId);
            ...
            if (badges.isEmpty()) {
                Label icon = new Label("🏅");
                icon.setStyle("-fx-font-size: 36px;");
                Label msg = new Label("Aucun badge obtenu pour le moment.");
                msg.setStyle("-fx-font-size: 14px; -fx-text-fill: #8a9bb5; -fx-padding: 12 0 0 0;");
                badgesContainer.setAlignment(javafx.geometry.Pos.CENTER);
                badgesContainer.getChildren().addAll(icon, msg);
                return;
            }
            ...
            flow.setHgap(12);
            flow.setVgap(12);
            flow.setPadding(new javafx.geometry.Insets(8));

            for (CertificationFinale cf : badges) {
                VBox card = new VBox(6);
                card.setAlignment(javafx.geometry.Pos.CENTER);
                card.setStyle(
                        "-fx-background-color: #f7f9ff;" +
                                "-fx-border-color: #dee8ff; -fx-border-width: 1;" +
                                "-fx-padding: 16 20; -fx-background-radius: 8;"
                );

                Label badgeIcon = new Label(cf.getBadge());
                badgeIcon.setStyle("-fx-font-size: 28px;");

                String dateStr = cf.getDateEmission() != null
                        ? cf.getDateEmission().toLocalDateTime()
                        .format(java.time.format.DateTimeFormatter.ofPattern("dd/MM/yyyy"))
                        : "—";
                Label dateLabel = new Label(dateStr);
                dateLabel.setStyle("-fx-font-size: 11px; -fx-text-fill: #8a9bb5;");

                card.getChildren().addAll(badgeIcon, dateLabel);
                flow.getChildren().add(card);
            }

            badgesContainer.getChildren().add(flow);
