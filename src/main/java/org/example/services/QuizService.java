...
public class QuizService {
        ...
        try {
            String sql =
                    "SELECT COUNT(*) FROM quiz " +
                            "WHERE titre = ? AND cours_associe_id = ?";
    ...
    public boolean existeDejaSaufId(String titre, int coursId, int id) {
        try {
            String sql =
                    "SELECT COUNT(*) FROM quiz " +
                            "WHERE titre = ? AND cours_associe_id = ? AND id != ?";
            PreparedStatement ps = cnx.prepareStatement(sql);
            ps.setString(1, titre.trim());
            ps.setInt(2, coursId);
            ps.setInt(3, id);
    ...
    public void ajouter(Quiz quiz) throws Exception {
        ...
        String sql =
                "INSERT INTO quiz (titre, type_quiz, duree, " +
                        "score_minimum, cours_associe_id) VALUES (?,?,?,?,?)";
        PreparedStatement ps = cnx.prepareStatement(sql);
    ...
    public void modifier(Quiz quiz) throws Exception {
        ...
        String sql =
                "UPDATE quiz SET titre=?, type_quiz=?, duree=?, " +
                        "score_minimum=?, cours_associe_id=? WHERE id=?";
        PreparedStatement ps = cnx.prepareStatement(sql);
        ps.setString(1, quiz.getTitre());
        ps.setString(2, quiz.getTypeQuiz());
        ps.setInt(3, quiz.getDuree());
